...
public class EmailConfig {
    ...
    public JavaMailSender mailSender() {        //JavaMailSender 인터페이스를 구현한 객체를 빈으로 등록하기 위함.
        ...
        mailSender.setPort(587);                                        // TLS(Transport Layer Security) port
        mailSender.setUsername(username);                               // host email
        ...
        javaMailProperties.put("mail.smtp.ssl.protocols", "TLSv1.3");   // 사용할 ssl 프로토콜 버젼

        mailSender.setJavaMailProperties(javaMailProperties);           // mailSender에 만들어준 properties 넣기
